...
export const checkNoteLimits = async (c: Context, next: Next) => {
  ...
  const noteCountResult = await db
    ...
    .from(notes)
    .where(and(eq(notes.userId, userId), or(isNull(notes.deleted), eq(notes.deleted, false))));
...
export const checkStorageLimits = (expectedFileSizeBytes: number) => {
  return async (c: Context, next: Next) => {
    ...
    const currentStorageBytes = storageResult[0]?.totalBytes
      ? Number(storageResult[0].totalBytes)
      : 0;
